...
def weightUpdate(data,test,p):
    for epoch in range(p.nEpoches):
        ...
        for batch in range(int(data.shape[0]/p.batchsize)):
            ...
            p.dw = p.learning_rate * (grad_w + p.alpha * p.dw
                                      - p.L2_reg * p.weight - p.L1_reg * np.sign(p.weight))
...
def imageReconstruction(data,test,weight,visBias,hidBias,index,CDtype,layer):
    ...
    for l in range(layer-1,-1,-1):
        weight, visBias, hidBias = loadParams('params/params%d%sL%d.pkl'%(index,CDtype,l))
...
def generation(sample, weight, visBias, hidBias, nCD, visType = 'binary', hidType = 'binary'):
    hid=sample
...
#        vis = expit(np.dot(sampling(hid), weight.T)+ visBias)
#    elif hidType == 'prob':
#        vis = expit(np.dot(hid, weight.T) + visBias)
#    else:
#        print('hidType shoould be binary or prob')
#        exit()
#
#    for cd in range(nCD):
#        if visType == 'binary':
#            hid = expit(np.dot(sampling(vis), weight) + hidBias)
#        elif visType == 'prob':
#            hid = expit(np.dot(vis, weight) + hidBias)
#        else:
#            print('visType shoould be binary or prob')
#            exit()
#
#        if hidType == 'binary':
#            vis = expit(np.dot(sampling(hid), weight.T)+ visBias)
